          ...
          {approvals.map((r) => (
            <div key={r.id} className="border-b border-amber-500/20 bg-amber-500/[0.06] px-3.5 py-2.5">
              ...
              <div className="mt-2 flex items-center gap-1.5">
                ...
                {r.project_id && (
                  <span className="ml-auto rounded bg-raise px-1.5 text-[9.5px] text-muted">
                    {r.project_id}
                  </span>
                )}
              </div>
...
export function AccountChip() {
  ...
  return (
    <div
      className="flex items-center gap-1.5 rounded-full border border-line bg-page py-[2px] pl-[2px] pr-2"
      // The reason matters when there is no login: "gh is not installed" and
      // "you are not signed in" need different things from you.
      title={signedIn ? `Signed in to GitHub as ${user.login}` : user?.reason || 'Checking GitHub…'}
    >
      <span
