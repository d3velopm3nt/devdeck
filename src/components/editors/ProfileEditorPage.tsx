...
import { EditorShell, Field, type EditorPageProps } from './EditorShell'
...
export function ProfileEditorPage(props: EditorPageProps<Params>) {
  const state = useApp()
  ...
  if (!profile) {
    return <div className="p-6 text-muted">Profile not found (it may have been deleted).</div>
  }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 pt-1">
        <span className="text-[11px] font-medium text-muted">Steps (run top to bottom)</span>
        {steps.length === 0 && (
          <div className="rounded border border-dashed border-line2 px-3 py-4 text-center text-[12px] text-muted">
            No steps yet — add them below.
          </div>
        )}
        {steps.map((s, i) => (
          <div key={i} className="flex items-center gap-2 rounded bg-soft px-2 py-1.5 text-[12.5px]">
            <span className="w-5 text-[10px] text-muted">{i + 1}.</span>
            <span className="flex-1 text-body">{stepLabel(s, state)}</span>
            <button className="inline-flex items-center px-1 text-muted hover:text-ink disabled:opacity-30" disabled={i === 0} onClick={() => move(i, -1)} title="Move up">
              <Icon name="arrow-up" size={14} />
            </button>
            <button className="inline-flex items-center px-1 text-muted hover:text-ink disabled:opacity-30" disabled={i === steps.length - 1} onClick={() => move(i, 1)} title="Move down">
              <Icon name="arrow-down" size={14} />
            </button>
            <button className="px-1 text-muted hover:text-err" onClick={() => setSteps(steps.filter((_, j) => j !== i))} title="Remove">
              ×
            </button>
          </div>
        ))}
