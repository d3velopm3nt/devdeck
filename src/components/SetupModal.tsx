      ...
      <div className="max-h-[86vh] w-full max-w-[540px] overflow-y-auto rounded-xl border border-slate-700 bg-[#11141c] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 px-5 py-4">
          {setup.tools.length > 0 && (
            <div>
              <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-wider text-slate-500">Required tools</div>
              <div className="flex flex-col gap-1.5">
                {setup.tools.map((t) => (
                  <div key={t.binary} className="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-[#151923] px-3 py-2">
                    <span className={`h-2 w-2 shrink-0 rounded-full ${t.installed ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                    <span className="text-[12.5px] font-medium text-slate-200">{t.name}</span>
                    <span className="truncate font-mono text-[11px] text-slate-500">{t.pkg_id}</span>
                    <span className={`ml-auto shrink-0 rounded-full px-2 py-0.5 text-[10.5px] ${t.installed ? 'bg-emerald-500/12 text-emerald-400' : 'bg-amber-500/15 text-amber-300'}`}>
          ...
          {setup.steps.length > 0 && (
            <div>
              <div className="mb-1.5 text-[10.5px] font-semibold uppercase tracking-wider text-slate-500">Setup steps</div>
              <div className="flex flex-col gap-1.5">
                {setup.steps.map((s) => (
                  <div key={s.run} className="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-[#151923] px-3 py-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="truncate font-mono text-[11px] text-slate-500">{s.run}</div>
                    </div>
                    <span className={`ml-auto shrink-0 rounded-full px-2 py-0.5 text-[10.5px] ${s.done ? 'bg-emerald-500/12 text-emerald-400' : 'bg-amber-500/15 text-amber-300'}`}>
